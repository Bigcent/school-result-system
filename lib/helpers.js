...
// ─── Auto-generated remarks based on average ───
export function getTeacherRemark(average) {
  if (average === null) return "";
  if (average >= 80) return "An excellent performance. Keep it up!";
  if (average >= 70) return "Very good performance. Keep up the good work.";
  if (average >= 60) return "Good performance. Can do better with more effort.";
  if (average >= 50) return "Fair performance. Needs to work harder.";
  if (average >= 40) return "Below average. Must sit up and work harder.";
  return "Poor performance. Needs much more effort and attention.";
}

export function getHeadTeacherRemark(average) {
  if (average === null) return "";
  if (average >= 80) return "Outstanding result. A pride to the school.";
  if (average >= 70) return "A very good result. Well done.";
  if (average >= 60) return "A good result. Put in more effort.";
  if (average >= 50) return "A fair result. More dedication is needed.";
  if (average >= 40) return "Below average. Needs serious improvement.";
  return "A poor result. Parents should please assist.";
}
